...
function main() {
  ...
  // var texturesDir = "http://127.0.0.1:8887/assets/SolarSystemScopePlanetTextures/" 
  var texturesDir = "../assets/SolarSystemScopePlanetTextures/" 
  ...
  var perspectiveMatrix = utils.MakePerspective(60, w/h, 0.1, 1000.0);
  ...
  sun.src = texturesDir+"Sun.jpg";

  mer = new Image();
  mer.txNum = 1;
  mer.onload = textureLoaderCallback;
  mer.src = texturesDir+"Mercury.jpg";

  ert = new Image();
  ert.txNum = 2;
  ert.onload = textureLoaderCallback;
  ert.src = texturesDir+"EarthDay.jpg";

  mon = new Image();
  mon.txNum = 3;
  mon.onload = textureLoaderCallback;
  mon.src = texturesDir+"Moon.jpg";

  ven = new Image();
  ven.txNum = 4;
  ven.onload = textureLoaderCallback;
  ven.src = texturesDir+"VenusAtmosphere.jpg";

  mar = new Image();
  mar.txNum = 5;
  mar.onload = textureLoaderCallback;
  mar.src = texturesDir+"Mars.jpg";

  jup = new Image();
  jup.txNum = 6;
  jup.onload = textureLoaderCallback;
  jup.src = texturesDir+"Jupiter.jpg";

  sat = new Image();
  sat.txNum = 7;
  sat.onload = textureLoaderCallback;
  sat.src = texturesDir+"Saturn.jpg";

  uranus = new Image();
  uranus.txNum = 8;
  uranus.onload = textureLoaderCallback;
  uranus.src = texturesDir+"Uranus.jpg";

  neptune = new Image();
  neptune.txNum = 9;
  neptune.onload = textureLoaderCallback;
  neptune.src = texturesDir+"Neptune.jpg";

  
  // calisto = new Image();
  // calisto.txNum=10;
  // calisto.onload=textureLoaderCallback;
  // calisto.src=texturesDir+"calisto.jpg";


  // ioj=new Image();
  // ioj.textNum=11;
  // ioj.onload=textureLoaderCallback;
  // ioj.src=texturesDir+"io.jpg";

  // mimas=new Image();
  // mimas.textNum=12;
  // mimas.onload=textureLoaderCallback;
  // mimas.src=texturesDir+"mimas.jpg";

  // titan= new Image();
  // titan.textNum=13;
  // titan.onload=textureLoaderCallback;
  // titan.src=texturesDir+"titan.jpg";
  ...
  uranusOrbitRadius = 32;

  neptuneOrbitRadius = 40;
...
titSpeed=0.1;
    ...
    objects.forEach(function(object) {
      ...
      // gl.useProgram(sunNode.drawInfo.programInfo);
      
      // var projectionMatrix = utils.multiplyMatrices(viewProjectionMatrix, sunNode.worldMatrix);
      // var normalMatrix = utils.invertMatrix(utils.transposeMatrix(sunNode.worldMatrix));
      ...
      // gl.bindVertexArray(sunNode.drawInfo.vertexArray);

      // gl.uniform1i(textLocation, 1);

      // gl.drawElements(gl.TRIANGLES, sunNode.drawInfo.bufferLength, gl.UNSIGNED_SHORT, 0 );
      ...
      // gl.uniform1i(program.textLocation, 1);
    ...
    // for(i = 0; i < planetWorldMatrix.length; i++){
    //   var viewWorldMatrix = utils.multiplyMatrices(viewMatrix, planetWorldMatrix[i]);

      // var viewWorldMatrix = utils.multiplyMatrices(viewMatrix, planetWorldMatrix);
      // var projectionMatrix = utils.multiplyMatrices(perspectiveMatrix, viewWorldMatrix);
      // gl.uniformMatrix4fv(matrixLocation, gl.FALSE, utils.transposeMatrix(projectionMatrix));
      

      // gl.uniformMatrix4fv(normalMatrixPositionHandle, gl.FALSE, utils.transposeMatrix(cubeNormal));
